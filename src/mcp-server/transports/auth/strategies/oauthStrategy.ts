...
export class OauthStrategy implements AuthStrategy {
  ...
  constructor(
    @inject(AppConfig) private readonly config: typeof ConfigType,
    @inject(Logger) private readonly logger: typeof LoggerType,
  ) {
  ...
  async verify(token: string): Promise<AuthInfo> {
    ...
    try {
      ...
      if (this.config.oauthIssuerUrl) {
        verifyOptions.issuer = this.config.oauthIssuerUrl
      }
      if (this.config.oauthAudience) {
        verifyOptions.audience = this.config.oauthAudience
      }
