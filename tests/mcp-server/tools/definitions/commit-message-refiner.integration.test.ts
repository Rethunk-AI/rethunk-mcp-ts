...
describe.skipIf(!hasApiKey)(
  'commitMessageRefiner - Integration Tests',
  () => {
    beforeAll(() => {
      if (!hasApiKey) {
    ...
    it(
      'should format a free-form summary into a conventional commit message using gpt-5-nano',
      async () => {
        const commitSummary =
        ...
        const result =
          await commitMessageRefiner.refineCommitMessage(
            commitSummary,
            context,
          );
    ...
    it(
      'should handle various commit summary formats',
      async () => {
        const testCases = [
    ...
    it(
      'should reject empty or whitespace-only responses from LLM',
      async () => {
